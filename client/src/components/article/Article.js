        ...
        <>
          <div className="d-flex justify-content-between">
            <div className="border-bottom ">
              <p className="display-3 me-4 text-capitalize fw-semibold">{state.title}</p>
              <span className="py-3">
                <small className=" text-secondary me-4 fw-medium">
          ...
          <div>
            ...
            <div className="comments my-4">
              {state.comments.length === 0 ? (
                <p className="text-center fs-5 pt-5">No comments yet...</p>
              ) : (
                state.comments.map((commentObj, ind) => {
                  return (
                    <div key={ind} className="bg-light  p-3 rounded-2">
                      <p
                        className="fs-4"
                        style={{
                          color: "dodgerblue",
            ...
            {currentUser.userType === "user" && (
              <form onSubmit={handleSubmit(writeComment)}>
                ...
                />
                <button type="submit" className="btn float-end text-white" style={{backgroundColor:"#4b189a"}}>
                  Send <IoMdSend className="fs-3 text-white" />
                </button>
              </form>
            )}
          </div>
